...
int main(){
    ...
    int sum_Y =  0;             //  Σy
    int sum_XY = 0;             //  Σ(xy)
    int sum_X_cuadrado = 0;     //  Σ(x^2)
    int cuadrado_sum_X = 0;     //  (Σx)^2
    double m = 0;                //  Pendiente
    double b = 0;                //  Intersección en y
    ...
    m = ((cantDatos * sum_XY) - (sum_X * sum_Y)) / ((cantDatos * sum_X_cuadrado) - cuadrado_sum_X);

    b = (sum_Y - (cantDatos * sum_X)) / cantDatos;

    cout << endl << "ΣX = " << sum_X << " ΣY = " << sum_Y << " ΣXY = " << sum_XY << endl;

    cout << "Σ(xy) = " << sum_XY << " Σ(x^2) = " << sum_X_cuadrado << " (Σx)^2 = " << cuadrado_sum_X << endl;
