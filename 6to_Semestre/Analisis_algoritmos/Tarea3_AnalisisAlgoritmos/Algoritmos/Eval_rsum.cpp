...
double Rsum(const vector<double>& a, int n);
...
int main() {
    ...
    for (size_t i = 0; i < cantidad_elementos.size(); i++) {
        ...
        Rsum(polinomio_actual, cantidad_elementos[i]);
    ...
    ofstream archivo_excel("../data/Eval_Rsum.csv");
    ...
    if(archivo_excel.is_open()){
        ...
        cout << "\nSe ha generado el archivo 'Eval_Rsum.csv'." << endl;
    } else {
